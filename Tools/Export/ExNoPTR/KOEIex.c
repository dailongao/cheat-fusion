...
#define ALIGN(adr,n) ((adr+(n-1)) & (~(n-1)))
#define MINEXCOUNT 2

static int IsScriptStart(unsigned int adr)
{
	return 1;
}
		...
		for(fseek(rom_fp,cur,0);cur<ex_end && cur<filesize-4;cur+=(4-check),fseek(rom_fp,cur,0) ) {		
			...
			for ( check = 0 ; check < 4 ; check++ ) {
				...
				if(found){
					...
					if(sect_flag==1){
						if(strcmp(str,"{end}")==0) {
							sect_end=(cur+(4-check));
							if(found_count>=MINEXCOUNT){
								strcat(buffer,"{end}\n");
								fprintf(out_fp,"#### %d <#JMP($%X,$%X)>####\n%s\n",txt_number++,sect_start,sect_end,buffer);
							}
							else{//返回句子开头,往后检测
								cur=sect_start+1-(4-check);
							}
							sect_flag=0;found_count=0;end_count=1;
						}
						else {
							if(strchr(str,':') && strchr(str,'{') && strchr(str,'}')){//如果有link符号
								...
								else if(strcmp(str,"{CTRLHIRA}")==0){
									kana_stat = 0;
								}
								else if(strcmp(str,"{CTRLKATA}")==0){
									kana_stat = 1;
								}
								else if(strstr(str,"|||")!=0){
									char *tmp = strstr(str,"|||");
									if(kana_stat) str=tmp+3;
									else *tmp=0;
									found_count++;
								}
								else{
